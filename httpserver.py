...
def get_file_list(directory_path):
    try:
        ...
        print(f"Error while getting file and directory list: {e}")
        return {"files": [], "directories": []}

def generate_html(file_list, base_url):
...
def handle_client_request(client_socket):
    ...
    if request_data:
        ...
        op_type = process_url(url);

        # Implement logic based on the HTTP method
        if method == "GET":

            if op_type == 'view':
                directory_path = r"C:\Users\Asus\Desktop\Computer Networks\CS305_Project\CS305_Project\data"  # Using a raw string
                name = extract_name_from_url(url)
                new_directory_path = os.path.join(directory_path, name)
                files = get_file_list(new_directory_path)
                ...
                file_path = os.path.join(directory_path, name,filename)
